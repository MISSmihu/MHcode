...
// LastError returns the most recent controller or WebView2 error. Upstream
// terminated the entire host process for these errors, which prevents an
// embedding application from recovering or showing a useful error message.
func (e *Chromium) LastError() error {
	e.errorMu.Lock()
...
func (e *Chromium) Embed(hwnd uintptr) bool {
	...
	if e.LastError() != nil || e.webview == nil || e.controller == nil {
		return false
	}
...
// Close shuts down the controller without terminating the embedding process.
func (e *Chromium) Close() error {
	e.shuttingDown = true
	var closeErr error
	if e.controller != nil {
		closeErr = e.controller.Close()
	}
	if e.webview != nil {
		e.webview.Release()
		e.webview = nil
